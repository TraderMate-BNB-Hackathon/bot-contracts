...
contract BotAggregator is IRouter, AccessControl, Ownable, ReentrancyGuard {
    ...
    bytes32 public maintainerRole =
        keccak256(abi.encodePacked("MAINTAINER_ROLE"));
    bytes4 private adapterSwapSelector =
        bytes4(
            keccak256(bytes("swap(address,address,address,uint256,uint256)"))
        );
    ...
    function removeMaintainer(address maintainer) external onlyOwner {
        require(
            hasRole(maintainerRole, maintainer),
            "does not have maintainer role"
        );
        _grantRole(maintainerRole, maintainer);
    }
    ...
    function query(
        ...
        for (uint256 i = 0; i < adpts.length; i++) {
            uint256 amountOut = IAdapter(adpts[i]).query(
                tokenIn,
                tokenOut,
                amountIn
            );
    ...
    function _swap(
        ...
        for (uint256 i = 0; i < adpts.length; i++) {
            amounts[i + 1] = IAdapter(adpts[i]).query(
                trade.path[i],
                trade.path[i + 1],
                amounts[i]
            );
    ...
    function swap(
        ...
        if (trade.path[0] == WETH) {
            require(
                ...
                "trade.amountIn must be at least equal to msg.value"
            );
